...
description: 실제 상품 사진·공급처 URL·쿠팡 경쟁상품과 리뷰를 근거로 상품기획과 콘텐츠기획을 분리하고 사용자 승인을 받은 뒤, 제품 비주얼은 이미지·GIF·영상으로 만들고 카피·정보는 편집 가능한 HTML로 조립해 소재별 QA와 통합 QA를 수행한다. 제품 사실·동일성 잠금, Browser Use 조사, 브랜드·오퍼, 구매 서사, imagegen, 반응형 HTML, 광고표현·접근성 검수가 필요할 때 사용한다.
---

# 쿠팡 상세페이지: 제품 → 상품기획 승인 → 콘텐츠기획 승인 → 소재 제작·QA → HTML 조립·통합 QA
...
상품기획은 `무엇을 누구에게 어떤 오퍼로 팔 것인가`를 잠근다. 브랜드는 그 결정을 반복해서 기억시키는 시스템이며 상품기획 안에 포함한다.
...
사용자 승인 없이 승인 기록 명령을 실행하지 않는다. 승인 후 상품기획이 바뀌면 기존 승인은 오래된 상태가 되어 자동 차단된다.
...
승인된 상품기획을 입력으로 `output/planning/content-plan.json`을 만든다. 추천 제품명·검색형 상품명, 리뷰 기반 구매 불안, 핵심 소구, 보류 소구, 관리·사이즈·구성·주의, 장별 카피·근거·자산·QA 기준을 모듈로 기록한다. 카피는 HTML에서 수정 가능한 `headline`과 `body`로 분리한다.
...
[references/ui-design-system.md](references/ui-design-system.md), [references/product-truth-and-asset-strategy.md](references/product-truth-and-asset-strategy.md), [references/commercial-ad-benchmark.md](references/commercial-ad-benchmark.md), [references/conditional-typography-pass.md](references/conditional-typography-pass.md)를 따른다.
...
2. 실제 SKU와 구조가 다르면 생성 반복 대신 동일성 게이트로 회귀해 촬영을 요청한다.
3. 각 파일을 `output/content-assets/manifest.json`에 안정적 ID, SHA-256, 대체 텍스트, 주장·근거·계보 ID와 함께 등록한다.
4. `output/qa/material-qa.json`에서 모듈별 기술·제품 동일성·주장-근거·카피 정확성·시각 품질을 자동·육안으로 검사한다.
5. 한 모듈이 실패하면 해당 소재만 수정하고 다시 QA한다. 소재 QA가 하나라도 실패하면 HTML 조립을 시작하지 않는다.
...
- 상품기획: `product-plan.json`, 현재 해시에 묶인 사용자 승인
...
- 후속 실행: 실사진 촬영·교체 목록, GIF 계획
- 검수: 소재별 자동·육안 QA, 조립 후 통합 자동·육안 QA, 재생성 로그

최종 보고 첫 줄에는 `편집 가능한 상세페이지: <project-root>/output/html/detail-page.html`을 적는다. 이어서 상품기획·콘텐츠기획 승인 상태, 소재 QA·통합 QA 결과, 남은 미확인 정보, 실사진·GIF·영상 우선순위만 간결하게 적는다.
