...
`output/planning/content-plan.json`은 승인된 제품기획의 SHA-256을 참조하고 다음을 정의한다.
...
콘텐츠기획은 제품기획 승인 후에만 만들 수 있다. 콘텐츠기획도 사용자가 현재 파일을 명시적으로 승인해야 한다. 제품기획이나 콘텐츠기획이 승인 후 변경되면 기존 승인 해시는 오래된 상태가 되어 다음 단계가 차단된다.

승인된 콘텐츠기획을 화면으로 옮기기 전에 `output/planning/visual-storyboard.json`을 만든다. 스토리보드는 모듈마다 주장·근거·자산 ID, 필수 시각 단서, 모델·샷·크롭, 치수 기준점과 수치형 AC를 잠근다. 스토리보드 자동 검증을 통과하지 않으면 ImageGen과 HTML 조립을 시작하지 않는다.
...
- HTML 제목과 중요 본문은 실제 360px·800px 브라우저 좌표에서 줄바꿈·행수·잘림·겹침을 검사한다. DOM 문자열이나 임의 폭 스크린샷만으로 통과시키지 않는다.
- 모든 모듈은 `data-claim-ids`, 모든 시각 자산은 `data-asset-id`를 가진다. 실제 좌표에서 모듈 순서 100%, claim/asset 연결 100%, 피사체 가시율 95%, 핵심 부위 가시율 100%를 검사한다.
...
6. `claim_visual_relevance`: 필수 시각 단서를 직접 확인한 육안 점수 80/100 이상
...
- 제품기획·콘텐츠기획 정렬
- 전체 구매 흐름과 모듈 순서
- 승인 스토리보드와 DOM 모듈 순서·claim/asset ID 일치
- 피사체·핵심 부위 bbox 크롭 가시율
- 브랜드·시각 일관성
- 모바일 반응형과 가로 스크롤
- 360px·800px의 단어 중간 분리, 한 글자 고아행, 허용 행수 초과, 글자·행 잘림과 겹침
- 대체 텍스트·시맨틱 제목·영상 제어
- 전체 문맥에서의 광고 표현
- 채널 규격과 정적 대체 가능성
...
node <skill-dir>/scripts/collect_html_typography_metrics.mjs --cdp <endpoint> --url <detail-page-url> --screenshots <project-root>/output/qa/typography-screenshots --output <project-root>/output/qa/typography-metrics.json
...
node <skill-dir>/scripts/collect_visual_layout_metrics.mjs --cdp <endpoint> --url <detail-page-url> --storyboard <project-root>/output/planning/visual-storyboard.json --screenshots <project-root>/output/qa/visual-layout-screenshots --output <project-root>/output/qa/visual-layout-metrics.json
