...
def test_project_index_lists_current_stage_and_never_overwrites_existing_project(tmp_path: Path):
    store = load_module().ProjectStore(tmp_path)
    store.create_project("summer-mask-001", "여름 스포츠 마스크", "coupang", "standard")
    ...
    with pytest.raises(FileExistsError):
        store.create_project("summer-mask-001", "덮어쓰기", "coupang", "standard")
...
def test_update_project_rejects_schema_or_identity_changes_and_path_traversal(tmp_path: Path):
    ...
    project = store.create_project("summer-mask-001", "여름 스포츠 마스크", "coupang", "standard")
    ...
    with pytest.raises(ValueError, match="3~64자"):
        store.create_project("a", "너무 짧은 ID", "coupang", "standard")
    with pytest.raises(ValueError, match="탐색 방식"):
