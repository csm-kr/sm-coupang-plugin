...
- 기준일: 2026-07-20
- 현재 버전: `coupang-commerce-automation` v0.2.0+codex.20260720005143
- 현재 위치: **Phase 1 — 도매꾹·쿠팡 소싱 브라우저 headless 전환 완료, 팔토시·깔창 원문·현재가·동일성 실제 재수집 대기**
...
2026-07-20에는 사용자의 병행 작업을 방해하던 브라우저 포커스 의존성을 제거했다. 도매꾹은 별도 임시 프로필의 로컬 headless Chrome에 Browser Harness를 연결하고, 쿠팡은 `headless=True`의 임시 `nodriver` 세션을 사용한다. 한글 PowerShell 표준입력을 UTF-8로 정규화한 실제 Browser Harness 스모크에서 `example.com` 연결·4프레임 기록·소유 프로세스 정리를 확인했고 소싱 테스트 53개가 통과했다. 이 전환은 런타임 구현 완료이며 팔토시·깔창의 실제 원문·현재가 재수집을 대신하지 않으므로 기존 `PRICE_REVIEW_BLOCKED`는 유지한다.

같은 날 고배수 소싱의 사용자 목표를 ADR-022로 다시 잠갔다. 입력은 선택 카테고리·도매꾹 개당 공급가 상한·최소 가격 배수이며, 판매 근거가 있는 동일 1개 구성 쿠팡 상품이 지정 배수 이상으로 판매되는 pair가 하나라도 있으면 탐색 일치다. 리뷰 1개 이상 또는 만족 인원 100명 이상 라벨을 판매 흔적의 대리 신호로 쓰고, 더 싼 다른 등록은 발견 pair의 탈락 근거로 사용하지 않는다. React 입력·pair 비교 화면, 필터 JSON·HTML, 플러그인 사본을 갱신해 소싱 53개·플러그인 31개 테스트를 통과했고 800px·360px 브라우저 QA에서 가로 넘침 0을 확인했다. `HIGH_MARKUP_DISCOVERY`의 전체 가격 승인·`SHORTLIST` 비승격 게이트는 유지한다.
