...
- 기준일: 2026-07-20
- 현재 버전: `coupang-commerce-automation` v0.2.0+codex.20260719101427
- 현재 위치: **Phase 1 — 깔창(project-20260719-084800-654) 공개 도매 후보 10개 실제 조사 완료, Browser Use·표시형 Chrome 시작 차단으로 Best·현재가·동일성 재수집 대기**
...
1. 사용자가 Chrome의 `chrome://inspect/#remote-debugging`에서 원격 디버깅을 허용하면 깔창 프로젝트의 패션잡화/화장품 Best 풀과 공급처 구매 증분부터 재수집한다. 허용 전에는 공개 웹 후보를 Best 표본이나 검증 공급조건으로 승격하지 않는다.
2. 표시형 Chrome을 시작할 수 있는 로컬 환경에서 깔창 10개 후보와 팔토시 10개 후보의 쿠팡 현재가·리뷰·완전 동일성을 홈 선진입 후 직렬 재수집한다. 허용 전에는 상품 선택 게이트를 열지 않는다.
