...
> 문서 탐색: [현재 상태](../STATUS.md) · [소싱 프로세스](SOURCING-PROCESS.md) · [현재 실행 결과 HTML](../reports/2026/2026-07-16/resourcing-exact-identity-relaxed/report.html) · [보고서 규칙](REPORTS.md)
...
현재 기준선은 소싱 테스트 `26 passed`와 플러그인 검증 통과다. CAPTCHA 또는 로그인이 나오면 자동 입력하거나 우회하지 말고 사람이 직접 완료하거나 실행을 중단한다.
...
4. 실행 중 연 탭의 target ID를 기록하고, 수집을 끝내면 해당 탭만 모두 닫는다. 사용자가 원래 열어 둔 탭이나 다른 세션의 탭은 닫지 않는다.
5. 중단·타임아웃·예외가 발생해도 `finally`에서 같은 탭 정리를 수행하고 종료 결과를 실행 기록에 남긴다.
...
표준 통과 기준은 공급조건 검증 완료, 할인 후 현재 실판매가가 확인된 동일 묶음 비교 표본 5개 이상, 정상가 마진 40% 이상, 판매가 10% 하락 후 마진 30% 이상이다. 판매량순 상위 10개의 일반 로켓이 3개 이하면 허용하고 4개 이상이면 경쟁 과다로 제외한다. 판매자로켓은 진입 가능하다.
...
후속 선택에서는 아이스 쿨링 스카프가 재검증 후보가 됐지만 MOQ와 쿠팡 현재 실판매가가 초기 입력과 달라 `PRICE_REVIEW_BLOCKED`로 되돌아갔다. 이력은 [선택 재검증 보고서](../reports/deprecated/2026/2026-07-16/sourcing-selection-cooling-scarf/selection-decision.html)에 보관됐고, 현재 판단은 [동일상품 우선 재소싱 결과](../reports/2026/2026-07-16/resourcing-exact-identity-relaxed/report.html)를 기준으로 한다.
