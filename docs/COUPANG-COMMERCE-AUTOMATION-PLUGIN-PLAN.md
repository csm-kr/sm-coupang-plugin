...
7. 후보별 도매꾹 URL, 쿠팡 근거 URL, 판매 근거 현재가·수익률 최저~최고, 고가 판매 근거, 추천가와 마진을 HTML·JSON으로 제공하고 사용자 선택을 기다린다.
...
- 판매 근거가 있는 현재가 최저~최고, 탐색 수익률 최저~최고와 `high_price_reference`를 실제 상품 링크가 있는 HTML·JSON 보고서로 생성
