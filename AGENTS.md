...
이 저장소는 도매 상품 소싱부터 쿠팡 시장·마진 검증, 사용자 선택, 상세페이지·GIF·영상·HTML 제작과 게시 전 QA까지 연결하는 Codex 플러그인을 개발한다. 일반 소싱 외에 도매꾹 Best에서 사용자 지정 개당 공급가 상한·최소 가격 배수와 리뷰/만족 판매 근거를 충족하는 쿠팡 동일 1개 상품 pair를 찾는 탐색 프로필을 제공하며, 탐색 일치는 전체 소싱 검증 전 자동 승격하지 않는다.
...
- 도매꾹 Best는 사용자 Chrome과 분리된 임시 로컬 headless Chrome에 Browser Harness를 연결해 표본화하고 회차 종료 시 소유 프로세스와 프로필을 정리한다.
- 고배수 탐색은 도매꾹 개당 공급가 상한과 최소 가격 배수를 입력받는다. 쿠팡 동일 1개 구성의 현재 실판매가가 기준 배수 이상이고 리뷰 1개 이상 또는 만족 인원 100명 이상 라벨이 있는 pair가 하나라도 있으면 탐색 일치이며, 더 싼 다른 등록은 이 pair의 탈락 근거로 사용하지 않는다.
- 쿠팡은 임시 프로필의 headless `nodriver` Chrome으로 홈 선진입 후 판매량순 검색을 직렬 처리하고 종료한다. headless 실패 시 표시형 브라우저로 자동 전환하지 않는다.
