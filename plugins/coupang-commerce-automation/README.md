...
UI 밖에서 상품·가격 선택을 이미 끝냈다면 상품기획의 `앞 단계 완료 확인`을 체크해 기존 `coupang-product-planning` 실행을 열 수 있다. 이 확인은 앞 단계 완료·승인을 대신하지 않으며 스킬이 SKU·가격·공급처 근거를 다시 검증한다. 사용자는 사이즈·실측, 구성, 소재, 관리법을 필수로 입력하고 실제 SKU·라벨 이미지는 단계 안 드롭존에 놓는다. 폴더 안 기존 이미지는 썸네일과 확대 화면으로 미리보며 이미지 경로를 입력하지 않는다. 경쟁사 별점 1~3점 저평점 리뷰는 상품기획 스킬이 Browser Harness로 직접 조사한다.
