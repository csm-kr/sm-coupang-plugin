...
소싱의 사용자 선택 카테고리는 `전체`, `패션잡화/화장품`, `의류/언더웨어`, `출산/유아동/완구`, `가구/생활/취미`, `스포츠/건강/식품`, `가전/휴대폰/산업`이며 선택 입력이다. 미선택 시 `전체`와 6개 대분류를 순환한다. `도매꾹 개당 공급가 상한`과 `최소 가격 배수`는 필수 양수 입력이며 3배·4배 같은 값을 직접 넣을 수 있다. 탐색 방식은 `high-markup` 하나만 허용하고 일반 소싱은 고배수 pair의 내부 재검증 단계로만 사용한다.
