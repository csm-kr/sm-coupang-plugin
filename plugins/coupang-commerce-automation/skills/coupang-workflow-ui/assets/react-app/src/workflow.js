...
export const STAGES = [
  {
    ...
    inputs: [
      field("category", "찾고 싶은 카테고리", "text", { placeholder: "예: 여름 생활용품" }),
      field("sourcingMode", "탐색 방식", "select", {
        options: [
          ["standard", "일반 소싱"],
          ["high-markup", "도매꾹 Best 고배수 탐색"],
        ],
      }),
...
function specialistFor(state, stage) {
  if (stage.id === "sourcing" && state.stageData.sourcing?.inputs?.sourcingMode === "high-markup") {
    return "coupang-best-high-markup-sourcing";
  }
  return stage.skill;
}
...
export function buildCodexPrompt(state, stageId) {
  ...
  const knownInputs = stage.inputs
